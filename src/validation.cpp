...
{
    // Tail emission. Subsidy halves on schedule but never below nSubsidyTail.
    // Pays for security after the halvings would otherwise zero it out.
    Assert(nHeight >= 0);
    static constexpr CAmount nSubsidyTail = 8 * COIN;
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    const uint256 seed_key{truenorth::SeedKeyForChild(pindex->pprev)};
    if (!CheckBlock(block, state, params.GetConsensus(), seed_key, !fJustCheck, !fJustCheck)) {
        if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
...
// seed_key is the RandomX seed for `block`. Get it from
// truenorth::SeedKeyForChild(pprev). Don't pass kGenesisSeed by default.
// Once the chain gets past block 2112 the seed changes per epoch, and
// using the wrong one silently breaks consensus.
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, const uint256& seed_key, bool fCheckPOW = true)
{
    // Check proof of work matches claimed amount
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(seed_key), block.nBits, consensusParams))
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const uint256& seed_key, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!CheckBlockHeader(block, state, consensusParams, seed_key, fCheckPOW))
        return false;
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        // Get prev block index up front -- we need it to derive the
        // per-epoch RandomX seed key for CheckBlockHeader's PoW check below.
        // (Originally CheckBlockHeader ran first; reordering means we may
        // surface prev-not-found / bad-prevblk on a header that *also* has
        // bad PoW, instead of the bad-PoW error. Both still reject the
        // header and misbehave the peer; semantics-equivalent.)
        ...
        const uint256 seed_key{truenorth::SeedKeyForChild(pindexPrev)};
        if (!CheckBlockHeader(block, state, GetConsensus(), seed_key)) {
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    const uint256 seed_key{truenorth::SeedKeyForChild(pindex->pprev)};
    if (!CheckBlock(block, state, params.GetConsensus(), seed_key) ||
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // Look up parent via the block index to derive the RandomX seed key.
        // ProcessNewBlock fires for fresh inbound blocks; an unknown parent
        // here means we received a block before its header chain -- defer
        // to AcceptBlock to handle that path, but for the CheckBlock PoW
        // pre-check we fall back to kGenesisSeed (anti-DoS pre-screen only;
        // the full path will re-validate with the right seed).
        CBlockIndex* pprev{nullptr};
        {
            auto mi{m_blockman.m_block_index.find(block->hashPrevBlock)};
            if (mi != m_blockman.m_block_index.end()) pprev = &mi->second;
        }
        const uint256 seed_key{truenorth::SeedKeyForChild(pprev)};
        bool ret = CheckBlock(*block, state, GetConsensus(), seed_key);
        if (ret) {
            // Store to disk
    ...
    const bool check_merkle_root)
{
    ...
    // TestBlockValidity asserts above that block.hashPrevBlock == tip, so
    // `tip` is the right pprev for the seed key derivation.
    const uint256 seed_key{truenorth::SeedKeyForChild(tip)};
    // For signets CheckBlock() verifies the challenge iff fCheckPow is set.
    if (!CheckBlock(block, state, chainstate.m_chainman.GetConsensus(), seed_key, /*fCheckPow=*/check_pow, /*fCheckMerkleRoot=*/check_merkle_root)) {
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        const uint256 seed_key{truenorth::SeedKeyForChild(pindex->pprev)};
        if (nCheckLevel >= 1 && !CheckBlock(block, state, consensus_params, seed_key)) {
